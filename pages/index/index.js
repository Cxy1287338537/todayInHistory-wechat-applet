...
Page({
  ...
  logStorage: function () {
    var node = this;
    wx.request({
      url: 'http://119.23.53.56:8080/mysql?appId=100&app_key="'+this.data.date+'"'
    });
    this.data.logs.remove(this.data.date);
    this.setData({
      logs: [this.data.date].concat(this.data.logs).slice(0, 10)
    }, function () {
      wx.setStorage({
        key: 'logs',
        data: node.data.logs,
